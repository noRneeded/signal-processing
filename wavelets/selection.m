...
% Evaluating central frequencies, passbands, and reconstruction errors
for iWavelet = 1 : length(WAVELET_LIST)
    maxDecompLevel(iWavelet, 1) = wmaxlev(N_SAMPLES, char(WAVELET_LIST(iWavelet)));
    [waveletCoef{iWavelet, 1}, waveletCoef{iWavelet, 2}] = ...
        wavedec(signal, maxDecompLevel(iWavelet), char(WAVELET_LIST(iWavelet)));
    [waveletEnergy{iWavelet, 1}, waveletEnergy{iWavelet, 2}] = ...
        wenergy(waveletCoef{iWavelet, 1}, waveletCoef{iWavelet, 2});
